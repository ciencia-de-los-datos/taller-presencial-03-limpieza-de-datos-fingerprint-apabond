...
def create_fingerprint(df):
    ...
    df["key"] = df["key"].apply(lambda x: [stemmer.stem(word) for word in x]) #aplica una palabra general para las diferentes conjugaciones
    df["key"] = df["key"].apply(lambda x: sorted(set(x))) #ordenan las palabras y eliminar duplicados en una misma cadena de texto
    df["key"] = df["key"].str.join(" ") #para que las palabras se separen por espacio
...
def generate_cleaned_column(df):
    ...
    df["cleaned"] = df["key"].map(key_dict) #crea la nueva columna, la columna limpia
